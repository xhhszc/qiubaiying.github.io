...
$$E[Y-Y^{a=0}|A,Z]=A(\beta_0+\beta_1 Z)$$或$$E[Y^{a=1}-Y^{a=0}|A=1, Z]=\beta_0 + \beta_1 Z$$

其中$\beta_1$等效于工具变量$Z$对于结果变量$Y$的效应系数``(The $\beta_1$ quantifies additive effect modification by $Z$.)``，即：
（i） 当$Z=0$时：$E[Y^{a=1}-Y^{a=0}|A=1, Z=0]=\beta_0$；

（ii） 当$Z=1$时：$E[Y^{a=1}-Y^{a=0}|A=1, Z=1]=\beta_0+\beta_1$
